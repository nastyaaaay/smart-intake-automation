...
| Классификация/автоответ | Локальная LLM через [Ollama](https://ollama.com) (`gpt-oss:20b`, HTTP Request к `/api/chat` + JSON-schema `format` для гарантированно валидного JSON, `think: "low"` — у reasoning-моделей вроде gpt-oss рассуждения нельзя выключить совсем, только понизить уровень) | без внешних API, без ключей и лимитов; изначально пробовали Google Gemini free tier — уткнулись в постоянный `429` (бесплатная квота фактически недоступна для аккаунтов не из поддерживаемого региона), поэтому переехали на локальную модель |
...
Первая версия этого раздела утверждала, что `think: "low"` ненадёжен (примерно 1 из 5 прогонов жалобы на возврат денег классифицировался как `spam` вместо `complaint`) и поэтому был выбран `"medium"`. Это оказалось артефактом теста, а не свойством модели: тестовые запросы отправлялись через `curl -d` с инлайн-кириллицей в git-bash на Windows, который битово портит UTF-8 не в консоли (там всё было видно нормально), а в самом теле запроса — модель на входе реально получала испорченный текст и разумно результат "не знаю, похоже на мусор → spam".

После пересборки теста на файловых JSON-пейлоадах (`curl --data-binary @file.json`, без инлайн-кириллицы в shell) — как напрямую к Ollama, так и через реальный n8n-webhook — `think: "low"` дал 9 из 9 верных классификаций подряд на той же жалобе. При этом `"low"` в 3-4 раза быстрее `"medium"` (~13с против 36-60с на запрос), что для UX формы существенно. Поэтому рабочая версия использует `think: "low"`.
...
Для проверки shared-secret на вебхуке и авторизации статус-бота (см. раздел "Безопасность") нужны переменные окружения, так что на практике запуск выглядит так:

```bash
INTAKE_SHARED_SECRET=ваш-секрет MANAGER_CHAT_ID=chat_id-менеджера N8N_BLOCK_ENV_ACCESS_IN_NODE=false npx n8n
```

(В n8n Variables — платная фича, поэтому секреты передаются через `$env`, а не `$vars`. А `N8N_BLOCK_ENV_ACCESS_IN_NODE=false` нужен, потому что начиная с какой-то версии n8n по умолчанию блокирует нодам доступ к переменным окружения — без этого флага ноды "Check Shared Secret" и "Check Authorized" не смогут прочитать `$env` и упадут с `access to env vars denied`.)
...
**Путь заявки в n8n** — Webhook → проверка секрета → классификация Ollama → Google Sheets → приоритизация → уведомление менеджеру или автоответ:

![Пайплайн в n8n](docs/demo-n8n-pipeline.png)
...
- **Статус-бот защищён проверкой chat_id** (`MANAGER_CHAT_ID`, нода "Check Authorized" в `workflow-status-bot.json`) — раньше `/status <id>` отвечал любому, кто написал боту, а `id` заявки (timestamp) легко угадать в узком диапазоне; посторонний мог узнать имя, контакт и суть чужой заявки. Найдено повторным аудитом — [issue #6](https://github.com/nastyaaaay/smart-intake-automation/issues/6). Пофиксено.
- **Санитизация перед записью в Google Sheets** (`Parse AI Classification`) — поля, начинающиеся с `=`, `+`, `-`, `@`, экранируются ведущим апострофом, чтобы заявитель не мог внедрить формулу (`=HYPERLINK(...)` и подобное), которая выполнится при открытии таблицы менеджером. Тоже из issue #6.
